// §1.5, §1.6, §1.8 deep dive — Vision dashboard
...
export function VisionScreen() {
  ...
  return (
    ...
    >
      ...
      {/* §1.5 Target audience */}
      <SectionHeader title="§1.5 Who Circle is for" hint="Six core audiences" />
      ...
      {/* §1.6 + Roadmap */}
      <SectionHeader title="§1.6 Quantitative Roadmap" hint="Egypt → MENA → Global" />
      ...
      <SectionHeader title="§1.8 Long-Term Vision (10 Years)" hint="The non-enshittification covenant" />
      <div className="space-y-3 mb-12">
        {LONG_TERM.map((l) => (
          <GlassCard key={l.t}>
      ...
      <div className="text-center pt-4">
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          <Link to="/covenant"    className="px-4 py-2 rounded-full bg-secondary text-secondary-foreground text-xs"><Target className="w-3 h-3 inline mr-1" /> §1 The Covenant</Link>
          <Link to="/identity"    className="px-4 py-2 rounded-full glass border border-border/40 text-xs"><Globe2 className="w-3 h-3 inline mr-1" /> §2 Brand</Link>
          <Link to="/roadmap"     className="px-4 py-2 rounded-full glass border border-border/40 text-xs"><TrendingUp className="w-3 h-3 inline mr-1" /> §34 Roadmap</Link>
          <Link to="/journeys"    className="px-4 py-2 rounded-full glass border border-border/40 text-xs"><UsersIcon className="w-3 h-3 inline mr-1" /> §35 Journeys</Link>
        </div>
      </div>
    </PageShell>
  );
